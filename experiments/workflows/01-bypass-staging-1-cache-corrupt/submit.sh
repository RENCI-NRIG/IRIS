...
# first, put data on the staging site
ssh uc-staging "mkdir -p ~/public_html/inputs"
scp job-wrapper.sh inputs/* uc-staging:~/public_html/inputs/

# clear all the caches
for CACHE in syr-compute-c2 unl-compute-c1 ucsd-compute-c3; do
    ssh $CACHE "sudo rm -f /cache/uc-staging-~$USER*"
done
