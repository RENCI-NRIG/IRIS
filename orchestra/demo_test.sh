...
# default 1 round = 4 workflows from each submit site
TEST_MODE=1
round=1
...
echo "python3 ${BASH_SOURCE%/*}/../es/iris-es-to-ml.py -s $(cat ${RESULT_DIR}/ts_start) -e $(cat ${RESULT_DIR}/ts_end)"
python3 ${BASH_SOURCE%/*}/../es/iris-es-to-ml.py -s $(cat ${RESULT_DIR}/ts_start) -e $(cat ${RESULT_DIR}/ts_end)
mv transfer-events.csv ${RESULT_DIR}/transfer-events.csv
python3 ${BASH_SOURCE%/*}/iris_gen_result.py $RESULT_DIR

#cp /root/console.log ${RESULT_DIR}/
cp ${BASH_SOURCE%/*}/test_env.sh ${RESULT_DIR}/
